...
# Total MediaPipe pose landmarks. The wire format below sends only the
# six this representation uses, and they are re-expanded into a full
# 33-landmark array so training and serving run the identical code.
N_POSE_FULL = 33
...
def build_sequence(npz_path, config):
    ...
    return _sequence_from_arrays(arrays, config)


def _sequence_from_arrays(arrays, config):
    """
    Turn raw landmark arrays into a model input.

    Split out from build_sequence so that background/"no sign" windows
    and, at inference time, live browser landmarks can go through
    exactly the same code path as training data. Duplicating this logic
    is what produced the v3 train/serve mismatch, so there is one
    implementation and everything routes through it.
    """
    pose = np.asarray(arrays["pose"], dtype=np.float32)
    left = np.asarray(arrays["left"], dtype=np.float32)
    right = np.asarray(arrays["right"], dtype=np.float32)
    pose_ok = np.asarray(arrays["pose_ok"], dtype=bool)
    left_ok = np.asarray(arrays["left_ok"], dtype=bool).copy()
    right_ok = np.asarray(arrays["right_ok"], dtype=bool).copy()
    meta = arrays["meta"]
...
def build_background_sequence(npz_path, config, rng=None):
    """
    Build a "no sign" sample from the idle part of a real recording.

    Every recording begins and ends with the signer sitting or standing
    still before and after the sign. Motion trimming throws that away —
    but it is exactly what a camera sees when nobody is signing, and it
    is real footage of real people rather than synthetic noise or an
    arbitrary "background" recorded elsewhere.

    Using it as a rejection class means the model learns to say "no sign
    detected" from genuine idle data, at no annotation cost.

    Returns None when a recording has no idle stretch long enough to
    sample from, which is common for the briskest signers.
    """
    with np.load(npz_path) as data:
        arrays = {key: data[key] for key in data.files}
    ...
    if pose.shape[0] < 24 or pose_ok.sum() == 0:
        return None

    start, end = _motion_interval(
        pose, pose_ok, config.motion_energy_keep, config.motion_pad_frames
    )

    # Candidate idle stretches: before the sign starts, and after it ends.
    candidates = []
    if start >= 12:
        candidates.append((0, start))
    if pose.shape[0] - end >= 12:
        candidates.append((end, pose.shape[0]))
    ...
    window = {
        key: (arrays[key][lo:hi] if key != "meta" else arrays[key])
        for key in arrays
    }

    # Trimming is disabled for the idle window: it is already the
    # segment we want, and running motion detection on near-stationary
    # frames would just re-expand it to the whole clip.
    idle_config = FeatureConfig(
        **{**config.describe(), "trim_to_motion": False}
    )

    return _sequence_from_arrays(window, idle_config)


def sequence_from_live_frames(frames, width, height, config):
    """
    Build a model input from landmarks captured live in a browser.

    This is the serving entry point, and it deliberately routes into the
    exact same `_sequence_from_arrays` used for training data. The v3
    system reimplemented normalization in TypeScript, which meant the
    browser reproduced a normalization bug independently of the training
    code and the two could drift apart silently. There is now one
    implementation.

    Wire format — one row per captured frame, 150 values, RAW and
    unnormalized, matching MediaPipe's own output:

        [0:24]    6 pose landmarks (11..16) as x, y, z, visibility
        [24:87]   left hand,  21 landmarks as x, y, z
        [87:150]  right hand, 21 landmarks as x, y, z

    A hand is absent when its 63 values are all zero, which is how
    MediaPipe's own "no hand detected" result is encoded.

    `width` and `height` are the video element's pixel dimensions, used
    for aspect correction — without them a 16:9 stream would be
    normalized differently in the browser than in training.

    Returns None when there is no usable pose.
    """
    frames = np.asarray(frames, dtype=np.float32)

    if frames.ndim != 2 or frames.shape[1] != 150 or frames.shape[0] == 0:
        raise ValueError(
            f"Expected (n_frames, 150) live frames, got {frames.shape}."
        )

    n = frames.shape[0]

    # Re-expand the six transmitted pose landmarks into a full 33-slot
    # array at their true MediaPipe indices, so _motion_interval and
    # _sequence_from_arrays can index them as they do during training.
    pose = np.zeros((n, N_POSE_FULL, 4), dtype=np.float32)
    pose_block = frames[:, :24].reshape(n, 6, 4)
    for slot, landmark_id in enumerate(POSE_IDS):
        pose[:, landmark_id, :] = pose_block[:, slot, :]

    left = frames[:, 24:87].reshape(n, 21, 3)
    right = frames[:, 87:150].reshape(n, 21, 3)

    # A landmark block of all zeros means MediaPipe reported no hand.
    left_ok = np.abs(left).sum(axis=(1, 2)) > 1e-6
    right_ok = np.abs(right).sum(axis=(1, 2)) > 1e-6

    # Pose is present when the shoulders carry any signal.
    pose_ok = np.abs(pose_block[:, :2, :3]).sum(axis=(1, 2)) > 1e-6
    ...
    arrays = {
        ...
        "left_ok": left_ok,
        "right_ok": right_ok,
        "meta": np.asarray([0.0, width, height, n], dtype=np.float32),
    }
